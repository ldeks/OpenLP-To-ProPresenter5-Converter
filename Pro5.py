...
class Pro5Writer: 
    ...
    def writeFile(self, directory, title, ccli_number, comments, copyright_year, copyright, authors):
        success = True

        # Prepare Header Block to write:

        to_write = ( HeaderBlock(Name          = xml_attr(title),
                             CCLILicenceNumber = xml_attr(ccli_number),
                             Notes             = xml_attr(comments),
                             CCLICopyRightInfo = xml_attr(copyright_year),
                             Publisher         = xml_attr(copyright),
                             Authors           = xml_attr(authors)) )
        ...
        try:
            ...
            f = open(directory + title.replace('/','') + '.pro5','w')
            f.write ( to_write + FooterBlock )
            f.close()
        except:
            print ('Oh dear. Something went wrong with writing "' + title + '".\nSorry.\n\n' +
